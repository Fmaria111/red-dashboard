...
<style>
    ...
    section[data-testid="stSidebar"] {{ background: linear-gradient(180deg, #1a0a0a 0%, #2d1111 100%); }}
    ...
    .stMetric > div > label {{ color: {C_TEXT} !important; font-size: 0.95rem; }}
    .stMetric > div > div {{ color: {C_WHITE} !important; font-size: 2rem; font-weight: 700; }}
    /* 表格 */
    ...
    /* KPI大数字卡片 */
    .kpi-card {{
        background: linear-gradient(145deg, #2d1111 0%, #3d1a1a 100%);
    ...
    .section-header {{
        ...
        border-bottom: 2px solid #5a2020;
    }}
    .section-icon {{
        font-size: 1.5rem;
    }}
    ...
    .gantt-plan {{
        background: linear-gradient(90deg, #5a2020, #8B0000);
        color: white;
        padding: 4px 12px;
        border-radius: 8px;
        font-weight: 600;
...
PLOT_LAYOUT = dict(
    ...
    plot_bgcolor=C_CARD,
    font=dict(color=C_WHITE, family="Microsoft YaHei, SimHei, sans-serif"),
...
# ────────────────── Plotly 通用布局（规划版）──────────────────
COLOR_BASELINE = "#FF6B6B"   # 现状-红色
COLOR_TARGET = "#4CAF50"     # 目标-绿色
COLOR_GAP = "#FFD700"        # 差距-金色
COLOR_BG2 = "#1a0a0a"        # 背景
COLOR_GRID = "#3d1a1a"       # 网格线
COLOR_TEXT2 = "#e0c0c0"       # 文字

PLOTLY_LAYOUT = dict(
    paper_bgcolor=COLOR_BG2,
    plot_bgcolor=COLOR_BG2,
    font=dict(family="Noto Sans SC, sans-serif", color=COLOR_TEXT2, size=12),
    xaxis=dict(gridcolor=COLOR_GRID, linecolor=COLOR_GRID),
    yaxis=dict(gridcolor=COLOR_GRID, linecolor=COLOR_GRID),
    legend=dict(bgcolor="rgba(26,10,10,0.8)", font=dict(color=COLOR_TEXT2)),
...
if page == "现状诊断":
    ...
    <div style="text-align:center; padding: 20px 0 10px 0;">
        <h1 style="color:{C_ACCENT}; font-size:2.6rem; margin-bottom:2px;">🔴 红色传播导航仪表盘 · 现状诊断</h1>
        <p style="color:{C_TEXT}; font-size:1rem;">《数链红星·星火篇》革命老区青少年红色文化触达率调研 · 2025</p>
    </div>
    <hr style="border:1px solid {C_DARK};"/>
    """, unsafe_allow_html=True)
    ...
    with col_a:
        ...
        fig_age.update_traces(textfont_color=C_WHITE, textinfo="percent+label")
    ...
    fig_ch = px.bar(
        ...
        title="信息获取渠道排名（多选）",
        color="人次", color_continuous_scale=["#5D4037", C_ACCENT],
    )
    ...
    fig_feel.update_traces(textfont_color=C_WHITE, textinfo="percent+label")
    ...
    fig_radar.add_trace(go.Scatterpolar(
        ...
        fill="toself",
        fillcolor="rgba(183, 28, 28, 0.4)",
        line=dict(color=C_ACCENT, width=2),
        marker=dict(color=C_ACCENT, size=8),
    ))
    fig_radar.update_layout(
        polar=dict(
            radialaxis=dict(visible=True, range=[0, 10], tickfont=dict(color=C_TEXT)),
            bgcolor=C_CARD,
            angularaxis=dict(tickfont=dict(color=C_WHITE, size=14)),
        ),
    ...
    with col_i1:
        ...
        fig_avatar = px.bar(
            ...
            color_continuous_scale=["#5D4037", C_ACCENT],
        )
    ...
    with col_i2:
        ...
        fig_equip = px.bar(
            ...
            title="创作设备需求（多选）",
            color="人次", color_continuous_scale=["#5D4037", C_GOLD],
        )
    ...
    st.markdown(f"""
    <hr style="border:1px solid {C_DARK};"/>
    <div style="text-align:center; color:{C_TEXT}; font-size:0.85rem; padding:10px 0;">
...
# ══════════════════════════════════════════════
        ...
        with col:
            st.markdown(
                ...
                <div class="info-card" style="border-left-color: #DC143C;">
                    ...
                    <span style="color:#FF6B6B;">{sol["base"]}</span>
                    <span style="color:#FFD700;"> → </span>
                    <span style="color:#4CAF50;">{sol["target"]}</span><br><br>
                    <b>✅ {sol["sol"]}</b><br>
                    <span style="color:#cc9999; font-size:0.82rem;">{sol["detail"]}</span>
                </div>
                ''',
                unsafe_allow_html=True
            )
    ...
    with res_col3:
        ...
        for role, count, desc in roles:
            st.markdown(
                f'''
                <div class="info-card" style="border-left-color: #5a2020; padding: 0.5rem 0.8rem; margin: 0.3rem 0;">
                    <b>{role}</b> × {count}<br>
                    <span style="color:#996666; font-size:0.78rem;">{desc}</span>
                </div>
                ''',
                unsafe_allow_html=True
            )
    ...
    with insight_col2:
        ...
        fig_scan = go.Figure(go.Bar(
            ...
            y=scan_dist.values,
            marker_color=[
                "#8B0000" if int(k) <= 5 else "#FF8C00" if int(k) <= 7 else "#4CAF50"
    ...
    with close_col2:
        st.markdown(
            ...
            <div class="kpi-card" style="border-color: #FF8C00;">
                ...
                <div style="color:#FF8C00; font-size:0.9rem;">
                    触达率→65%<br>
                    认知分→7.0<br>
                    痛点全面改善<br>
                    创作参与→45%
